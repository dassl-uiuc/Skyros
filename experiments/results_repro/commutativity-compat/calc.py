...
client = 10
workloads = ['compat', 'compatconflict']
codes = ['orig', 'curp', 'rtop', 'rtopcomm']
workloadnames = {}
workloadnames['compat'] = 'no-conflict'
workloadnames['compatconflict'] = 'zipfian'
run_ids =[ '1']
d = sys.argv[1]
...
for workload in workloads:
	for code in codes:
		...
		print(str(curx) + '\t' + workloadnames[workload] + '\t' + code + '\t' + str(normalized) + '\t' +  str(throughput_dict[code][workload]))
		files[code].write(str(curx) + '\t' +  workloadnames[workload] + '\t' + str(normalized) + '\t' +  str(throughput_dict[code][workload]) + '\n')
		curx += bar_width
	curx += (bar_width)
