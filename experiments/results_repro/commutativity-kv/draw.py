...
outfile = indir + '/kvcurp.eps'
c = postscript(title=outfile,  dimensions=[270, 170])
d = drawable(canvas=c, xrange=[0,66], yrange=[0,120], coord=[55,27], dimensions=[220,121])
tables = []
for code in codes:
	infile = indir + '/' + code
...
lw = 0.1
bw = 8
p.points(drawable=d, table=tables[0], xfield='c0', yfield='zero', style='label', labelfield='c1', labelanchor='c,c',labelsize=14, labelshift=[26,-13])
...
for i in [0, 1, 2]: 
	p.points(drawable=d, table=tables[i], xfield='c0', yfield='c3',  style='label', labelfield='c2', labelanchor='l,c', labelsize=14.0, labelshift=[-14, 12])
	pass
